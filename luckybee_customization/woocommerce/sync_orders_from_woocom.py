...
from luckybee_customization.luckybee_customization.hook_events.update_webhook_status import update_webhook_status
from luckybee_customization.luckybee_customization.hook_events.publish_item_to_woocom import get_woocommerce_settings
...
def get_orders():
    ...
    try:
        ...
        try:
            ...
            sales_order.payment_request_url = response.get('payment_url')
                ...
                try:
                    ...
                    for meta in item_details.get('meta_data', []):
                        if meta.get('key') == '_amz_asin':
                            ...
                            break 
                    frappe.log_error("asin",asin_value) 
            ...
            for data in response['meta_data']:
                if data['key'] == 'pickup_date':
                    due_date = data['value']
                    sales_order.delivery_date = frappe.utils.getdate(due_date)
                else:
